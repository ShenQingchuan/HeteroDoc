...
import type { Command, IEditorExtension, KeyboardShortcutCommand, NoArgsCommand } from '../../types'
...
export class TextIdent implements IEditorExtension {
  ...
  keymaps: () => Record<string, KeyboardShortcutCommand> = () => {
    const changeIndent = (type: 'increase' | 'decrease'): KeyboardShortcutCommand => {
      return ({ commands }) => {
        const foundParentTxtBlock = findParentNode(node => node.isTextblock)(this.core.view.state.selection)
        if (!foundParentTxtBlock)
          return false
