...
export const pluginResetStoredMarksOnDeleted = () =>
  new Plugin({
      ...
      if (prevState.selection.empty) {
        ...
        for (const step of curTr.steps) {
          if (
            ...
            currentStoredMarks?.some((curStoredMark) =>
              curTr.storedMarks?.includes(curStoredMark)
            )
          ) {
            currentStoredMarks
              .filter((curStoredMark) =>
                curTr.storedMarks?.includes(curStoredMark)
              )
        ...
        const currentStoredMarksSameInPrev =
          currentStoredMarks?.filter((curStoredMark) =>
            prevSelectionMarks.includes(curStoredMark)
          ) ?? []
        const isCurrentStoredMarksNodesDeleted =
          currentStoredMarksSameInPrev.length > 0 &&
          currentSelectionMarks.every(
            (curMark) => !prevSelectionMarks.includes(curMark)
          )
        if (isCurrentStoredMarksNodesDeleted) {
          currentStoredMarksSameInPrev.forEach((staleMark) =>
            newTr.removeStoredMark(staleMark)
          )
          return newTr
        }
      }
