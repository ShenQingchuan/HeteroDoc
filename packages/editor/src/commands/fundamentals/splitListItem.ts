...
export const splitListItem: Commands['splitListItem'] =
  ({ typeOrName }) =>
  ({ tr, state, dispatch, core }) => {
    ...
    if (dispatch) {
      ...
      const splittableMarkExtNames = core
        .getSplittedableMarks()
        .map((markExt) => markExt.name as string)
      const marks =
      ...
      const filteredMarks = marks.filter((mark) =>
        splittableMarkExtNames.includes(mark.type.name)
      )
