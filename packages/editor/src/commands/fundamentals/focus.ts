...
export const focus: Commands['focus'] =
  (args?: FocusArgs) =>
  ({ core, view, tr, dispatch }) => {
    ...
    const delayedFocus = () => {
      ...
      if (isiOS()) (view.dom as HTMLElement).focus()
      ...
      requestAnimationFrame(() => {
        if (!core.isDestroyed) {
          ...
          if (options?.scrollIntoView) core.commands.scrollIntoView()
        }
      })
    }
    ...
    const selection =
      resolveFocusPosition(tr.doc, position) || view.state.selection
