...
Cypress.Commands.add(
  'shouldExistMarkSelector',
  { prevSubject: true },
  (subject, markSelector) => {
    return cy.wrap(subject).get(markSelector).should('exist')
  }
)
Cypress.Commands.add(
  'shouldNotExistMarkSelector',
  { prevSubject: true },
  (subject, markSelector) => {
...
Cypress.Commands.add(
  ...
  (subject, markName) => {
    ...
    return cy
      ...
      .typeWithModKey('a')
      .wait(enoughWaitTime) // waiting for showing menu
      .get(menuBtnSelector)
      .click()
      .wait(enoughWaitTime)
      .get(menuBtnSelector)
...
Cypress.Commands.add(
  ...
  (subject, checkFunction, originalOptions = {}) => {
  ...
  (subject, selector, callback) => {
    cy.wrap(subject)
      .waitUntil(() =>
        cy
          .get(selector)
          .then((el) => (Cypress.dom.isAttached(el) ? el : undefined))
      )
      .then((el) => {
        if (el) {
          callback(el)
