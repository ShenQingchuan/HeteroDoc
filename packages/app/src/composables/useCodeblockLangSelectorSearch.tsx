...
const langIdToLangNameAliasMap = Object.fromEntries(
  Object.entries(langNameToLangIdAliasMap).map((entry) => entry.reverse())
)
...
export function useCodeBlockLangSearchOptions() {
  ...
  const filteredOptions = computed(() => {
    if (searchWord.value === '') {
      return [...searchInputOption, ...langOptions]
    }

    return [
      ...searchInputOption,
      ...langOptions.filter((option) =>
        option.label.includes(searchWord.value)
      ),
    ]
  })
